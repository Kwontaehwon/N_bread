...
const Event  = require('../config/database/models/event');
...
router.get('/popup/:recentId', async (req, res, next) => {    
    try{
        ...
        if(event == null){
            ...
            return jsonResponse(res, 404, `PopUp Event를 찾을 수 없습니다.`, true, event);
        }
        if(req.params.recentId == event.id){
            return jsonResponse(res, 300, `PopUp 다시보지 않기를 선택한 회원입니다.`, true, null);
        }
...
router.post('/create', async (req, res, next) => {
    try{
        ...
        return jsonResponse(res, 200, `Event id : ${event.id} 가 생성되었습니다.`, true);
    }
    catch (error){
...
router.post('/img/:eventId', upload.single('img'), async (req, res, next) => {
    try {
        const file = req.file;
        ...
        return jsonResponse(res, 200, `Event id : ${eventId} 에 이미지가 Update 되었습니다.`, true);
    }
    catch (error){
        logger.error(`${error}  [Event Img Create] POST /events/img/:eventId`);
        jsonResponse(res, 500, "[Event Img Create] POST /events/img/:eventId", false);
    }
});
